...
#ifdef MASMULLIN_CHANGE
		else if( style != SCE_SH_COMMENTLINE && style != SCE_SH_STRING 
		    && (styler.Match(i-1,"\n") || styler.Match(i-1," ") || styler.Match(i-1,"\t")))
		{
		if( styler.Match(i, "do ") || styler.Match(i, "if ") || styler.Match(i,"case ") 
			|| styler.Match(i, "do\n") || styler.Match(i, "if\n") || styler.Match(i,"case\n")
			|| styler.Match(i, "do\r") || styler.Match(i, "if\r") || styler.Match(i,"case\r")
			|| styler.Match(i, "do\t") || styler.Match(i, "if\t") || styler.Match(i,"case\t"))
		{
			levelCurrent++;
		}
		else if( styler.Match(i, "done ") || styler.Match(i, "fi ") || styler.Match(i,"esac ")
			|| styler.Match(i, "done\n") || styler.Match(i, "fi\n") || styler.Match(i,"esac\n") 
			|| styler.Match(i, "done\r") || styler.Match(i, "fi\r") || styler.Match(i,"esac\r")
			|| styler.Match(i, "done\t") || styler.Match(i, "fi\t") || styler.Match(i,"esac\t") )
		{
			isClose = true;
			levelCurrent--;
		}
		else if( styler.Match(i, "else ") || styler.Match(i, "elif ")
			|| styler.Match(i, "else\n") || styler.Match(i, "elif\n")
			|| styler.Match(i, "else\r") || styler.Match(i, "elif\r")
			|| styler.Match(i, "else\t") || styler.Match(i, "elif\t") )
		{
			isClose = true;
			isElse = true;
		}
